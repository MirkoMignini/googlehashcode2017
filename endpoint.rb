class Endpoint
  ...
  def initialize(index, input, file)
    ...
    @caches_by_latency = @connected_caches.select{ |v| v[:latency] < latency }.sort_by { |v| v[:latency] }
  end
  ...
  def get_best_cache_latency(video)
    @caches_by_latency.each do |cc_cache|
      return cc_cache[:latency] if cc_cache[:cache].include?(video)
  ...
  def add_video(video)
    @caches_by_latency.each do |cc_cache|
      return true if cc_cache[:cache].include?(video)
      return true if cc_cache[:cache].add_video(video)
    end
    false
  end

  def get_best_video_latency(video)
    @caches_by_latency.each do |cc_cache|
      return cc_cache[:latency] if cc_cache[:cache].include?(video)
    end
    latency
